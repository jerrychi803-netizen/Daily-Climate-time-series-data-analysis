# ==============================================================================
# Complete XGBoost Training and Evaluation Script for Delhi Climate Dataset
# ==============================================================================

# 1. Install & Load Required Packages
required_packages <- c("xgboost", "ggplot2", "dplyr", "lubridate")
...
# Clean train set: filter out aggregate rows ('Grand Total') and missing target values
...
# Combine datasets chronologically to construct lag & rolling features cleanly
...
# Split into clean train and test sets
...
# Convert feature matrices into XGBoost DMatrix format
dtrain <- xgb.DMatrix(
  data  = as.matrix(train_set[, feature_vars]),
  label = train_set$meantemp
)

dtest <- xgb.DMatrix(
  data  = as.matrix(test_set[, feature_vars]),
  label = test_set$meantemp
)

# ------------------------------------------------------------------------------
# 4. Train XGBoost Model
# ------------------------------------------------------------------------------
cat("Training XGBoost regression model...\n")
...
# Train with early stopping monitored on training loss
xgb_model <- xgb.train(
  params    = params,
  data      = dtrain,
  nrounds   = 300,
...
p1 <- ggplot(results_df, aes(x = Date)) +
  ...
  geom_line(aes(y = Forecast, color = "XGBoost"), size = 1, linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "black", "XGBoost" = "deepskyblue4")) +
  labs(
    title = "XGBoost Model: Out-of-Sample Prediction vs Actual",
...
print(xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 10))